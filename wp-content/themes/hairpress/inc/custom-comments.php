<?php
function hairpress_comment( $comment, $args, $depth ) {
		// get the layout
				...
				<div class="row">
					<div class="comment-head">
						<div class="<?php if ($depth > 1) { echo 'span' . ( $main_class_span - 5 ); } else { echo 'span' . ( $main_class_span - 4 ); }?>">
							...
							<div class="avatar-name">
								<?php printf(__( '<p class="name"><span>%s</span> said:</p>' , 'proteusthemes'), get_comment_author_link()) ?>
								<?php comment_reply_link(array_merge( $args, array('add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth']))) ?>
							</div>
						</div>
						<div class="span4">
							<div class="date">
								<?php /* translators: 1: date, 2: time */ printf( __('%1$s at %2$s', 'proteusthemes'), get_comment_date(),  get_comment_time()); ?><?php edit_comment_link(__('(Edit)', 'proteusthemes'),'  ','' ); ?>
							</div>
						</div>
					</div>
					...
					<div class="comment-awaiting-moderation">
						<div class="<?php if ($depth > 1) { echo 'span' . ( $main_class_span - 1 ); } else { echo 'span' . $main_class_span;;}?>">
							<?php _e( 'Your comment is awaiting moderation.' , 'proteusthemes') ?>
